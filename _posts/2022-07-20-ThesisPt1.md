...
Let $G$ be a group and $V$ be a finite-dimensional vector space over a field $k$. A (group) **representation** $\rho$ **over** $k$ is a group homomorphism, $$\rho: G \to \text{Aut}(V).$$
...
Let $G$ be a group, $n$ a positive integer, and $k$ a field. A **representation** $\rho$ **over** $k$ is a group homomorphism $$\rho: G \to GL_n(k).$$
...
Let $R$ be a ring with identity. A **left $R$-module** $M$ is an abelian group $(M, +)$ that has a "left multiplication" by $R$,'' i.e. an operation $$\cdot : R \times M \to M$$ which satisfies, for all $r, s \in R$ and $m,n \in M$,
...
A **representation of $G$ over $k$** is a $kG$-module. 
